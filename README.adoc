...
Implementors of the API are expected to supply implementations of `Response` and `ResponseBuilder` by providing a `SPIFactory` for their runtime. The `SPIFactory` is discovered using the default JDK service laoder. 

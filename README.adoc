...
A Health check for startness allows applications to define startup probes that are used for initial verification of the application before the liveness probe takes over.

The _@Startness_ annotation must be applied on a _HealthCheck_ implementation to define a startness check procedure, otherwise, this annotation is ignored.

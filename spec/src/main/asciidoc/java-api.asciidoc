...
* Startness checks defined with `@Startness` annotation
...
A Health check for startness allows applications to define startup probes that
...
The `@Startness` annotation must be applied on a `HealthCheck` implementation to define a startness check procedure, otherwise, this annotation is ignored.
...
Any enabled bean with a bean of type `org.eclipse.microprofile.health.HealthCheck` and `@Liveness`, `@Readiness`, or `@Startness` qualifier can be used as health check procedure.
